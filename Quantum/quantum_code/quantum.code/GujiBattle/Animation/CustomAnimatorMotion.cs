...
namespace Quantum.Custom.Animator
{
    public unsafe interface IAnimatorMotion
    {
        ...
        void GenerateBlendList(CustomAnimator* a, AnimatorLayer layer, AnimatorState state, FP weight, List<AnimatorRuntimeBlendData> list);
        FP CalculateLength(CustomAnimator* a, FP weight);
        void CalculateWeights(Frame f, CustomAnimator* a);
    }
    ...
    public abstract unsafe class AnimatorMotion : IAnimatorMotion
    {
        ...
        public virtual void GenerateBlendList(CustomAnimator* a, AnimatorLayer layer, AnimatorState state, FP weight, List<AnimatorRuntimeBlendData> list) { }
        public virtual FP CalculateLength(CustomAnimator* a, FP weight) { return FP._0; }
        public virtual void CalculateWeights(Frame f, CustomAnimator* a) { }
    }
    ...
    public unsafe class AnimatorClip : AnimatorMotion
    {
        ...
        public override void GenerateBlendList(CustomAnimator* a, AnimatorLayer layer, AnimatorState state, FP weight, List<AnimatorRuntimeBlendData> list)
        {
            FP time = FP._0;
            FP lastTime = FP._0;
        ...
        public override FP CalculateLength(CustomAnimator* a, FP weight)
        {
            if (data == null)
                return 0;
            else
        ...
        public override void CalculateWeights(Frame f, CustomAnimator* a)
        {
            //not implemented
        }
    }
    ...
    public unsafe class AnimatorBlendTree : AnimatorMotion
    {
        ...
        public override void GenerateBlendList(CustomAnimator* a, AnimatorLayer layer, AnimatorState state, FP weight, List<AnimatorRuntimeBlendData> list)
        {
            ...
            for (int i = 0; i < motionCount; i++)
                if (weights[i] > zf)
                    motions[i].GenerateBlendList(a, layer, state, weights[i], list);
        }
        ...
        public override FP CalculateLength(CustomAnimator* a, FP weight)
        {
            ...
            for (int i = 0; i < motionCount; i++)
                if (motions[i] != null && weights[i] > zf)
                    output += motions[i].CalculateLength(a, weights[i]);
            return output;
        }
            ...
            { // 2d blend calculation
              //ASUMPTION - blend variables are between -1 and 1
                ...
                for (int z = 0; z < motionCount; z++)
                {
                    ...
                    weights[z] = (lerpa + lerpb + lerpc + lerpd) / 4;
                    totalWeight += weights[z];
                }
            }
            else
            {
                FP blendParameter = *CustomAnimator.Variable(f, a, blendParameterIndex)->FPValue / FP._2 + FP._0_50;
                ...
                for (int z = 0; z < motionCount; z++)
                {
                    ...
                    weights[z] = FPMath.Lerp(weightA, weightB, lerp);
                    totalWeight += weights[z];
                }
            }
        }

...
namespace Quantum.Custom.Animator
{
    ...
    public unsafe class AnimatorTransition
    {
        ...
        public void Update(Frame f, EntityRef* entity, CustomAnimator* a, CustomAnimatorGraph graph, AnimatorState state, FP deltaTime)
        {
            ...
            bool noCurrentTransition = CustomAnimator.allowTransitionInterruption || a->to_state_id == 0;
            bool selfConditional = canTransitionToSelf;

            if (a->to_state_id == 0)
            {
                selfConditional = selfConditional || a->current_state_id != destinationStateId;
            }
            else
            {
                selfConditional = selfConditional || a->to_state_id != destinationStateId;
            }
            ...
            {
                FP time = a->to_state_id == 0 ? a->time : a->to_state_time;
                ...
                if (!hasExitTime || time > exitTime)
                {
                    ...
                    if (conditionsMet)
                    {
                        ...
                        else
                        {
                            ...
                            a->from_state_time = a->to_state_time;
                            a->from_state_last_time = a->to_state_last_time;
                            a->from_state_normalized_time = a->to_state_normalized_time;
                            a->from_length = a->to_length;
                        ...
                        if (s.GetLength(a) == 0)
                        {
                            s.motion.CalculateWeights(f, a);
                        }

                        a->to_length = s.GetLength(a);

...
public class CustomExportAnimations : MonoBehaviour
{
    ...
    public static void CreateAsset(CustomAnimatorGraphAsset dataAsset, AnimatorController controller)
    {
        ...
        for (int l = 0; l < layerCount; l++)
        {
            ...
            // DeLucas:  I use state machines for organizations.  This process should get all of the state machines and their states.
            List<ChildAnimatorState> stateList = new List<ChildAnimatorState>(controller.layers[l].stateMachine.states);
            GetStateCountFromStateMachines(controller.layers[l].stateMachine.stateMachines, stateList);


            newLayer.states = new CA.AnimatorState[stateList.Count + 1];//additional element for the any state
            ...
            // Gets each state from the different state machines


            for (int s = 0; s < stateList.Count; s++)
            {
                UnityEditor.Animations.AnimatorState state = stateList[s].state;
            ...
            for (int s = 0; s < stateList.Count; s++)
            {
                ...
                // Finds and assigns the state asset
                if (AssignStateAsset(state.behaviours, out newState.StateAsset))
                {
                    var stateA = UnityDB.FindAsset<GameAnimatorStateAsset>(newState.StateAsset.Id);
                    stateA.animatorState = state;
                    EditorUtility.SetDirty(stateA);
                }
                else
                {
                    Debug.Log("No state found for:  " + newState.name);
                }
            ...
            newLayer.states[stateList.Count] = anyState;
    ...
    private static bool AssignStateAsset(StateMachineBehaviour[] behaviours, out AssetRefGameAnimatorState stateAsset)
    {
        stateAsset = null;

        foreach (StateMachineBehaviour b in behaviours)
        {
            if (b is GameASBContainer)
            {
                var container = (GameASBContainer)b;
                stateAsset = new AssetRefGameAnimatorState() { Id = container.stateAsset.AssetObject.Guid };
                return true;
            }
        }
    ...
    private static void GetStateCountFromStateMachines(ChildAnimatorStateMachine[] stateMachines, List<ChildAnimatorState> stateList)
    {
        for (int i = 0; i < stateMachines.Length; i++)
        {
            stateList.AddRange(stateMachines[i].stateMachine.states);
            GetStateCountFromStateMachines(stateMachines[i].stateMachine.stateMachines, stateList);
        }
    }
    ...
    public static AnimatorData Extract(AnimationClip clip)
    {
        ...
        for (int i = 0; i < frameCount; i++)
        {
            ...
            // DELUCAS:  Small fix for one frame animations?
            if (frameCount == 1)
                percent = 1f;

...
public class CustomAnimatorGraphAssetEditor : Editor
{
  ...
  private void OnEnable()
  {
    asset = (CustomAnimatorGraphAsset)target;
        ...
        for (int i = 0; i < asset.Settings.layers.Length; i++)
        {
            for (int j = 0; j < asset.Settings.layers[i].states.Length; j++)
            {
                if (asset.Settings.layers.Length > 1)
                {
                    s.Add(asset.Settings.layers[0].name + "/" + asset.Settings.layers[i].states[j].name);
                }
                else
                {
                    s.Add(asset.Settings.layers[i].states[j].name);
                }
            }
        }
        stateNameArray = s.ToArray();
  }
  ...
  public override void OnInspectorGUI()
  {
    ...
    if (graph != null && graph.layers != null)
    {
      ...
      if (layerCount > 0)
      {
                ...
                stateSelection = EditorGUILayout.Popup("Current State", stateSelection, stateNameArray);

                GetLayerAndState(stateSelection, out AnimatorLayer layer, out AnimatorState state);
        ...
        if (layer != null)
        {
          ...
          if (stateCount > 0)
          {
            ...
            //AnimatorState state = StateToState(stateSelection);//  layer.states[selectedState];
    ...
    private void GetLayerAndState(int stateSelection, out AnimatorLayer layer, out AnimatorState state)
    {
        layer = null;
        state = null;
        for (int i = 0; i < asset.Settings.layers.Length; i++)
        {
            if (i < asset.Settings.layers[i].states.Length)
            {
                layer = asset.Settings.layers[i];
                state = layer.states[stateSelection];
                break;
            }
            else
            {
                stateSelection -= asset.Settings.layers[i].states.Length;
            }
        }
